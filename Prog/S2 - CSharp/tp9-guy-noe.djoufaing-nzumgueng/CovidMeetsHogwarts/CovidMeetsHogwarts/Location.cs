...
namespace CovidMeetsHogwarts
{
    public class  Location
    {
        ...
        void AddHuman(bool randomValues)
        {
            ...
            if (randomValues)
                newHuman  = new Human(alea(), alea(), alea());
            else
                newHuman = new Human(Human.GLOBAL_HYGIENE, Human.GLOBAL_SOCIAL_DISTANCE, Human.GLOBAL_TRAVELLING_RATE);

            int pos = alea(map.GetNodes().Count);
            newHuman.SetCurrentSpot(map.GetNodes()[pos]);
            humans[pos] = newHuman;
        }
