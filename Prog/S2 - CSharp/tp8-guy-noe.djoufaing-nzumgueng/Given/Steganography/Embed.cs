...
namespace Steganography
{
    public class Embed
    {
        ...
        {
            Dictionary<int[], int>.KeyCollection cle = Utils.QuantizationTable.Keys;
            int res = -1;

            foreach (var keyValue in Utils.QuantizationTable)
            {
                if (difference > keyValue.Key[0]-1 && difference < keyValue.Key[1]+1)
                {
                    res = keyValue.Value;
                    break;
                }
