...
def ischain(G, L):
    ...
    for i in range(1, len(L)):
        ...
        if new in G.adjlists[old] and new not in visited:
            old = new
            visited.append(old)
...
def alldoublets(G, start):
    ...
    """
    l, p = [], [0] * G.order
    id, p[id] = G.labels.index(start), True
    ...
    while not q.isempty():
        ...
        for adj in G.adjlists[c]:
            if not p[adj]:
                ...
                q.enqueue(adj)
    i = 1
    while i < G.order and p[i]:
        i += 1

    return (None, None) if (i == G.order)  else (G.labels[0], G.labels[i])
...
def longestdoublet(G):
    ...
    for lab in G.labels:
        ...
        (max, l) = (dst[c], [G.labels[i], G.labels[c]] )if dst[c] > max else (max, l)
