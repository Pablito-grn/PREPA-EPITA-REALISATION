...
#define PATH_GRID "./datas/tmp/grid_made_solved"
...
void writerff(int sudoku[9][9]){
	FILE * fPtr  = fopen(PATH_GRID, "w");

	for (size_t y = 0; y < 9; y++)
	{
		for (size_t x = 0; x < 9; x++)
		{
			fprintf(fPtr,"%d",sudoku[y][x]);
			if ((x + 1) %3 == 0 && x != 0)
			{
				fprintf(fPtr," ");
			}
...
void load_file(OCR* ocr, char* filename){
	...
	int grid[9][9] = {0};
	NeuralNet(grid, liste, ocr->added_loaded);
	char tab[9][10];

	for (int i = 0; i < 9; ++i)
		for (int j = 0; j < 9; ++j)
			tab[i][j] = grid[i][j] == 0 ? ' ' : (grid[i][j] + '0');
